...
class RCControllerProfile(CommandProfile):
    def __init__(self, dt, state_estimator, x_scale=1.0, y_scale=1.0, yaw_scale=1.0, probe_vel_multiplier=1.0):
        super().__init__(dt)
        self.state_estimator = state_estimator # 이 레포 -> state_estimator = cheetah_state_estimator.py 내의 StateEstimator 클래스
    ...
    def get_command(self, t, probe=False): # 부모 클래스 (CommandProfile)의 get_command는 이걸로 대체된다.

        command = self.state_estimator.get_command() # StateEstimator로부터 기본 명령을 받음, 실제로 command에는 
        ...
        # check for action buttons -> 여기에서 buttons는 보행 방식 선택할 때, 컨트롤러에서 오른쪽 아래 4개 버튼
