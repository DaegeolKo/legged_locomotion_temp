...
from genesis_lr.legged_gym.envs.base.legged_robot_config import LeggedRobotCfg, LeggedRobotCfgPPO
...
class LCMAgent():
    def __init__(self, cfg, se, command_profile):
        ...
        self.dt = self.cfg.control.dt * self.cfg.control.decimation
        self.timestep = 0
        ...
        joint_names = [        # specify yhe sequence of actions
            'FR_hip_joint',
            'FR_thigh_joint',
            'FR_calf_joint',
            'FL_hip_joint',
        ...
        for i in range(12):
            ...
            for dof_name in self.cfg.control.stiffness.keys():
                if dof_name in joint_name:
                    ...
                    self.d_gains[i] = self.cfg.control.stiffness[dof_name]
                    found = True
            if not found:
                self.p_gains[i] = 0.
                self.d_gains[i] = 0.
        ...
        self.commands = torch.zeros((1, self.num_commands))
        ...
        self.gait_indices = np.zeros(self.num_envs)
        self.clock_inputs = np.zeros((self.num_envs, 4))
    ...
    def get_obs(self):
        ...
        self.gravity_vector = self.se.get_gravity_vector().reshape(1, -1)
        ...
        self.commands[:, :] = torch.tensor(cmds[:self.num_commands], device=self.device, dtype=torch.float)
        if reset_timer:
            self.reset_gait_indices()
        #else:
        #    self.commands[:, 0:3] = self.command_profile.get_command(self.timestep * self.dt)[0:3]
        self.dof_pos = torch.from_numpy(self.se.get_dof_pos()).to(self.device)
        self.dof_vel = torch.from_numpy(self.se.get_dof_vel()).to(self.device)
        ...
        #     ((self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos).cpu().detach().numpy().reshape(1, -1),
        #     (self.dof_vel * self.obs_scales.dof_vel).cpu().detach().numpy().reshape(1, -1),
        #     torch.clip(self.actions, -self.cfg.normalization.clip_actions, self.cfg.normalization.clip_actions).cpu().detach().numpy().reshape(1, -1)
        # ]
        

        ob_list = [(self.body_linear_vel * self.obs_scales.lin_vel).reshape(1, -1),   # 3
           (self.body_angular_vel * self.obs_scales.ang_vel).reshape(1, -1),  # 3
           self.gravity_vector,                                              # 3
           (self.commands[:,:3] * self.commands_scale).cpu().detach().numpy(),
           ((self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos).cpu().detach().numpy().reshape(1, -1),
           (self.dof_vel * self.obs_scales.dof_vel).cpu().detach().numpy().reshape(1, -1),
           torch.clip(self.actions, -self.cfg.normalization.clip_actions, self.cfg.normalization.clip_actions).cpu().detach().numpy().reshape(1, -1)
          ]

        ob = np.concatenate(ob_list, axis=1)
    ...
    def publish_action(self, action, hard_reset=False):
        ...
        self.joint_pos_target[[0, 3, 6, 9]] *= 1.0
        # self.joint_pos_target[[0, 3, 6, 9]] *= -1
        self.joint_pos_target = self.joint_pos_target
        self.joint_pos_target += self.default_dof_pos.cpu().numpy()
        ...
        self.torques = (self.joint_pos_target - self.dof_pos.cpu().numpy()) * self.p_gains + (self.joint_vel_target - self.dof_vel.cpu().numpy()) * self.d_gains
